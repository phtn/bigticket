...
import {SelectAccount} from 'convex/accounts/d'
...
const useTicketManagement = (
  ...
  const isVip = useMemo(
    () => IsVIP(xEvent?.vip_list, xAccount?.email),
    [xEvent?.vip_list, xAccount?.email],
  )

  const hasClaimed = useMemo(
    () => HasClaimedTickets(isVip, xEvent?.vip_list, xAccount?.email),
    [isVip, xEvent?.vip_list, xAccount?.email],
  )
  ...
  const ticketCount = useMemo(
    () =>
      xEvent?.vip_list?.find((t) => t.email === xAccount?.email)
        ?.ticket_count ?? 0,
    [xEvent?.vip_list, xAccount?.email],
  )
...
const MediaContainer = memo(({xEvent, moments}: MediaContainerProps) => {
  ...
  const handleGetTickets = useCallback(async () => {
    ...
    return await ticketCart?.getBasicTicket(xAccount?.uid)
  }, [ticketCart, isVip, xAccount?.uid])
